...
7. Execute the locked experiment deterministically on a repository-bundled OHLC snapshot from the documented Kaggle source (Kaggle uploader-declared CC0 1.0; effective experiment period 2007-09-17–2025-12-31 after validation against available sessions), with provenance file and checksums as required in Phase 3.
...
12. The entire required workflow must function without an API key or AI provider. Optionally call `POST /api/research/interpret` after earlier phase gates; configuration uses server-only `THESISTRAIL_LLM_ENDPOINT`, `THESISTRAIL_LLM_MODEL`, and `THESISTRAIL_LLM_API_KEY` (never `NEXT_PUBLIC_*`). Absent key, timeout, provider error, or invalid schema must use a clearly labeled **rules-based fallback** never mislabeled as AI. Interpretation may restate the question and phrase the four locked ambiguity categories only. It must never fabricate metrics or conclusions, modify locked numeric defaults, execute tests, or write LEARN conclusions.
